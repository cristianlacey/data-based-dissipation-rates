...
This repository stores the deep neural network model trained to predict the normalized instantaneous dissipation rate profiles for premixed turbuent combustion. The model was trained using ```TensorFlow``` version 2.4.1. The Anaconda environment used to train and postprocess the DNN model is provided in ```tf-gpu.yml``` and can be imported with the command ```conda env create -n myenv -f tf-gpu.yml```.
